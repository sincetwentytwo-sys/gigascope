...
export const TICKERS: Ticker[] = [
  ...
  },
  {
    ...
    lastVerified: "2026-05-22",
    deepDive:
      "ASML is the rarest thing in capitalism: a genuine, durable, technical monopoly. EUV (Extreme Ultraviolet Lithography) is the only path to leading-edge logic at 7nm and below — and ASML is the only company in the world that ships EUV scanners at production volume. The barriers to entry are stacked: a CO₂ laser hitting tin droplets 50,000 times per second to generate plasma EUV light; Zeiss SMT mirrors with surface roughness measured in single atoms; a 30+ year supply-chain integration that the Dutch government, on advice of intelligence services, restricts on export. Canon's nano-imprint lithography is theoretically an alternative for some non-leading-edge nodes, but has no path to volume at 3nm and below.\n\nThe product line: NXE Low-NA EUV (TWINSCAN NXE:3800E, the current 0.33 NA workhorse at TSMC/Samsung/Intel/Hynix), and the new EXE:5000 High-NA EUV (0.55 NA, ~$380M per tool, ~16 systems shipped through 2026). High-NA EUV is what TSMC needs for N2 (2nm) and below to avoid moving to multi-patterning. The order backlog is so deep that ASML has multi-year visibility on bookings even through cyclical downturns.\n\nThe bear case has three legs: (1) China revenue collapse from Dutch export controls — ~25% of historic revenue zero'd out. (2) Cyclical capex digestion at customers — every fab is now spending $20B+ in capital, and any pause shows up as ASML order push-out. (3) Mature-node (DUV) competition is real from Nikon/Canon, though it affects only ASML's smaller-revenue segment.\n\nThe bull case is the most boring on the market: as long as Moore's-Law-equivalent scaling drives the AI-build-out, there is no substitute for ASML's tools. Service + spare parts revenue ($6B+ annualized) is growing as the installed base grows. There is no second EUV manufacturer in any credible 10-year forward window.",
    sources: [
  ...
  },
  {
    ...
    lastVerified: "2026-05-22",
    deepDive:
      "Rocket Lab is the closest public-market analog to SpaceX — but actually shipping. Electron has flown 40+ commercial missions with a credible reliability track record at the small-lift end of the market (~300 kg to LEO). Neutron, the medium-lift reusable vehicle in flight-test integration, is the binary catalyst on the equity. First flight is the single biggest variable in the company's 2026 valuation discussion: a successful maiden launch + first-stage recovery would arguably re-rate the entire small-cap-space cohort. Schedule slip is the bear case — Neutron has slipped multiple times from a 2024 target.\n\nWhat makes Rocket Lab structurally interesting beyond Neutron: the Space Systems segment is real, growing, and high-margin. RKLB ships satellite bus platforms (Photon), star trackers, deployable solar arrays, reaction-control system components, and end-to-end mission services. The acquisition of SolAero (solar arrays for satellites) + the integrated Photon spacecraft platform positions Rocket Lab as a small-/mid-sized full-stack space-systems prime — closer to a Northrop Grumman Spacecraft division than to a pure launch company. Defense + NRO customer mix is improving each contract cycle.\n\nThe bear case: Neutron schedule risk, ongoing cash burn until Neutron operational, valuation already prices first-flight success at meaningful probability. The bull case: at sustained Neutron cadence (target 12-24 launches/year by 2028), RKLB becomes the only credible US medium-lift alternative to Falcon 9 — and given national-security-launch contract structures favoring vehicle diversity, that alone justifies a meaningful market-cap multiple.",
    sources: [
  ...
  },
  {
    ...
    lastVerified: "2026-05-22",
    deepDive:
      "BYD is the most underappreciated company in global automotive — outside Asia. Vehicle deliveries surpassed Tesla on a quarterly basis multiple times in 2024-2025, and the company sells more pure-EV + plug-in hybrid vehicles than any other manufacturer in the world. The competitive moat is vertical integration that no Western automaker has approached: BYD makes its own LFP cells (Blade architecture is internally developed cell-to-pack), its own electric motors, its own power electronics, its own automotive-grade silicon carbide modules. The Yangwang U9 supercar showcases what this can produce — and the Seagull subcompact at ~$10K USD anchors the cost-leadership end of the lineup.\n\nGlobal expansion is the 2026-2028 growth story: Hungary (Szeged) plant + Brazil (Camaçari) + Thailand + Indonesia + Mexico. These are tariff-resilient routes around the EU CBAM + US 100% Section 301 tariffs that effectively lock BYD out of the US passenger market. In Europe specifically, BYD now outsells legacy German OEMs in select segments.\n\nThe bear case is severe. (1) Locked out of US passenger market by tariffs, with no clear path even under most-favorable trade policy. (2) Chinese domestic price war compressing margin per vehicle dramatically. (3) Geopolitical concentration — a single-country political shift could disrupt global expansion plans overnight. (4) Western OEMs racing to deploy their own LFP capacity (Tesla, LGES Holland MI) eats into BYD's cost advantage.\n\nThe bull case rests on Asia + Latin America + Middle East volume + the structural cost-curve advantage BYD has built. At sub-$15K BEV pricing with positive contribution margin, BYD defines the floor of global EV economics in a way that compresses every Western competitor's pricing power.",
    sources: [
  ...
  },
  {
    ...
    lastVerified: "2026-05-22",
    deepDive:
      "IBM is the most-dismissed AI-era name in the public market, and that dismissal is at least partly wrong. Two real business lines deserve attention: IBM Quantum and Red Hat / watsonx.\n\nIBM Quantum is the most-deployed gate-model quantum computing stack in the world. The Heron processor (156 qubits) is shipping in IBM Quantum System Two, the modular system architecture that the company is scaling toward Kookaburra (4,158 qubits, target 2026). Crucially, IBM has been publishing a multi-year roadmap that, so far, has actually been hit on schedule — a rarity in quantum computing. The competitive position vs trapped-ion (IONQ) and superconducting startups (RGTI) is: more shipped systems, more cloud accessibility (Qiskit Runtime), and a real path through error correction toward useful logical qubits.\n\nRed Hat is the de-facto hybrid-cloud Kubernetes infrastructure layer — and IBM's $34B acquisition of it in 2019 looks better every year. Every Fortune 500 datacenter modernization runs through OpenShift. watsonx is IBM's enterprise-LLM platform — initially dismissed as 'IBM is late' but increasingly winning bank, healthcare, and government accounts where data residency + on-prem deployment + audit trail matter more than benchmark scores. The open-weights Granite model family is IBM's contribution to the open LLM ecosystem.\n\nThe bear case: consulting services revenue is mature; quantum is still rounding-error on the income statement; growth has to come entirely from software. The bull case: IBM is the only public-market name that lets investors take meaningful, non-speculative quantum exposure inside a profitable, dividend-paying enterprise software company. The risk/reward is asymmetric in a way that pure-play quantum names (IONQ, RGTI, QBTS) cannot match.",
    sources: [
