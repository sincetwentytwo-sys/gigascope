  ...
  return (
    <div className="max-w-[1200px] mx-auto px-6 py-10">
      ...
      <div className="mt-4 h-1.5 bg-surface rounded-full overflow-hidden">
        <div className="h-full rounded-full" style={{ width: `${factory.progress}%`, background: company.color }} />
      </div>

      {/* Info Cards */}
      <div className={`grid gap-3 mt-8 ${extraEntries.length > 0 ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4" : "grid-cols-2 sm:grid-cols-4"}`}>
        {[
          ...
          ...extraEntries.map(([label, value]) => ({ label, value })),
        ].map((c) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-8">
        <div className="lg:col-span-2 h-[350px] sm:h-[450px] rounded-xl overflow-hidden">
          <SatelliteMapWrapper lat={factory.lat} lng={factory.lng} zoom={15} factoryColor={company.color} />
        </div>
            ...
            {factory.milestones.map((m, i) => (
              <div key={i} className={`relative pl-6 ${!m.done ? "opacity-40" : ""}`}>
                <div
                  ...
                  style={{
                    borderColor: m.done ? company.color : "var(--border)",
                    background: m.done ? company.color : "transparent",
                  }}
                />
            ...
            return (
              <div key={TIMELINE_YEARS[i]} className="flex-1 flex flex-col items-center justify-end h-full">
                ...
                <div
                  ...
                  style={{ height: heightPx, background: isLatest ? company.color : "#c7c7cc" }}
                />
